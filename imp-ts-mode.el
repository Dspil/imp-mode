...
(defun imp-ts-mode:is-conjunction (left right ref &optional invert)
  (let ((left-paren (format "(%s)" left))
        (right-paren (format "(%s)" right)))
    (if invert
        (or (equal (concat "!" left "&&" right) ref)
            (equal (concat "!" left-paren "&&" right) ref)
            (equal (concat "!" left "&&" right-paren) ref)
            (equal (concat "!" left-paren "&&" right-paren) ref))
      (or (equal (concat left "&&" right) ref)
          (equal (concat left-paren "&&" right) ref)
          (equal (concat left "&&" right-paren) ref)
          (equal (concat left-paren "&&" right-paren) ref)))))
...
(defun imp-ts-mode:check-if-rule (pre condition then-pre then-post else-pre else-post post)
  (let* ((pre-str (imp-ts-mode:stringify-to-logic pre))
    ...
    (when (not (equal then-post-str else-post-str))
      (imp-ts-mode:error then-post-start then-post-end "If rule violated:\nAssertions at the end of 'then' and 'else' branches should be syntactically equal." t)
      (imp-ts-mode:error else-post-start else-post-end "If rule violated:\nAssertions at the end of 'then' and 'else' branches should be syntactically equal."))
    (when (not (equal then-post-str post-str))
      (imp-ts-mode:error then-post-start then-post-end "If rule violated:\nAssertions at the end of 'then' branch and after the if statement should be syntactically equal." t)
      (imp-ts-mode:error post-start post-end "If rule violated:\nAssertions at the end of 'then' branch and after the if statement should be syntactically equal."))
    (when (not (imp-ts-mode:is-conjunction condition-str pre-str then-pre-str))
      (imp-ts-mode:error pre-start pre-end "If rule violated:\nAssertion entering 'then' branch should be the conjunction of the if condition and the assertion before the if statement." t)
      (imp-ts-mode:error then-pre-start then-pre-end "If rule violated:\nAssertion entering 'then' branch should be the conjunction of the if condition and the assertion before the if statement."))
    (when (not (imp-ts-mode:is-conjunction condition-str pre-str else-pre-str t))
      (imp-ts-mode:error pre-start pre-end "If rule violated:\nAssertion entering 'else' branch should be the conjunction of the negated if condition and the assertion before the if statement." t)
      (imp-ts-mode:error else-pre-start else-pre-end "If rule violated:\nAssertion entering 'else' branch should be the conjunction of the negated if condition and the assertion before the if statement."))))
...
(defun imp-ts-mode:check-skip-rule (node pre post)
  (when (not (equal (imp-ts-mode:stringify-to-logic pre)
    ...
    (imp-ts-mode:error (treesit-node-start (treesit-node-child node 1)) (treesit-node-end node) "Skip rule violation:\nAssertions before and after skip should be equal." t)))
